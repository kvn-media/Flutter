...
class AppRouter extends RouterDelegate <AppLink>
  ...
  bool _handlePopPage(Route<dynamic> route, result) {
    ...
    if (route.settings.name == KvnmediaPages.onboardingPath) {
      appStateManager.logout();
    }

    if (route.settings.name == KvnmediaPages.groceryItemDetails) {
      groceryManager.groceryItemTapped(-1);
    }

    if (route.settings.name == KvnmediaPages.profilePath) {
      profileManager.tapOnProfile(false);
    }

    if (route.settings.name == KvnmediaPages.raywenderlich) {
